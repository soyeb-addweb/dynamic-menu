...
(function ($) {
    ...
    function isV3() {
        return $('.e-n-menu').length > 0 || $('.menu-item-practice-areas').length > 0;
    }
    function selectors() {
        if (isV3()) {
            return {
        ...
        return (window.dynamicMenuData && dynamicMenuData.menu_selectors) ? dynamicMenuData.menu_selectors : {
            areas_we_serve: '.menu-item-areas-we-serve',
            practice_areas: '.menu-item-practice-areas'
        };
    }

    // ---------- Elementor v3 helpers ----------
    ...
    function applyCityRewriteV3(citySlug, cityName) {
        if (!citySlug) return;
    ...
    function init() {
        ...
        if (isV3()) {
            ...
            if (usingElementorMegaMenu) {
                $(document).off('click.ecAreasCityV2').on('click.ecAreasCityV2', (sel.areas_we_serve + ' .elementor-icon-list-items a'), function () {
                ...
                $(document).off('click.ecAreasCityV2').on('click.ecAreasCityV2', (sel.areas_we_serve + ' .sub-menu a'), function () {
                    var cityUrl = $(this).attr('href') || '';
                    var citySlug = cityUrl.split('/').filter(Boolean).pop();
                    var cityName = $(this).text();
                    try { sessionStorage.setItem('currentCitySlug', citySlug); } catch (e) {}
